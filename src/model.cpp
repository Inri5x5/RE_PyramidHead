...
model_t load_obj(const std::string &path, const glm::vec3 diffuse, const glm::vec3 specular, const glm::vec3 emissive, float shininess) {
    ...
    for (const tinyobj::shape_t &shape: shapes) {
        ...
        for (const tinyobj::index_t &index: shape.mesh.indices) {
            ...
            float *n =  &attrib.normals[3 * index.normal_index];
    ...
    for (const tinyobj::material_t &mat: materials) {
        ...
        glm::vec4 spec_col = { mat.specular[0], mat.specular[1], mat.specular[2], 1};

        mats.emplace_back(material_t{ tex, col, spec , spec_col });
    }

    return model_t{ meshes,mats,diffuse,specular,emissive,shininess };
}
